...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	meanSpeed := meanSpeed(action, duration) // Средняя скорость в км.ч
	return ((runningCaloriesMeanSpeedMultiplier * meanSpeed * runningCaloriesMeanSpeedShift / mInKm) * weight * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeed := meanSpeed(action, duration) // Средняя скорость в м/с
	return ((walkingCaloriesWeightMultiplier*weight +
		(math.Pow(meanSpeed*kmhInMsec, 2)/(height/100))*walkingSpeedHeightMultiplier*weight) *
		duration * minInH)
}
